...
import { withConnection } from "@/lib/db/simplified-connection"; // Using simplified connection
  ...
  try {
    ...
    try {
      ...
      if (!mongoose.connection || !mongoose.connection.db) {
        console.error(
          "[DIAGNOSTIC] Connection established but db object is not available"
        );
        return {
          success: false,
      ...
      console.log(
        "[DIAGNOSTIC] Available collections:",
      ...
      if (sampleUser) {
        ...
        console.log(
          ...
          sampleUser.hashedPassword?.length
        );
        console.log(
          "- HashedPassword starts with:",
          sampleUser.hashedPassword?.substring(0, 10)
        );
...
export async function diagnoseAuth(
  ...
  try {
    ...
    console.log(
      "[DIAGNOSTIC] User lookup completed in",
      Date.now() - startTime,
      "ms"
    );
    ...
    const validHashFormat =
      user.hashedPassword.startsWith("$2") && user.hashedPassword.length >= 50;
    console.log(
      `[DIAGNOSTIC] Password hash format looks valid: ${validHashFormat}`
    );
    console.log(
      `[DIAGNOSTIC] Password hash: ${user.hashedPassword.substring(0, 10)}...`
    );
    ...
    try {
      ...
      console.log(
        `[DIAGNOSTIC] Password comparison completed in ${
          Date.now() - bcryptStartTime
        }ms`
      );
    ...
    console.log(
      "[DIAGNOSTIC] User data being returned:",
